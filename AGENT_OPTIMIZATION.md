...
`cnctl` implements several patterns to optimize execution for LLM agents. These optimizations focus on minimizing token usage and providing deterministic, machine-readable interfaces.
...
- Application errors are caught and formatted as structured JSON:
  ```json
  {
    ...
    "hints": ["Run 'cnctl login' to authenticate."]
  }
  ```
